...
void MenuUpdate(GSGLOBAL* gsGlobal)
{
    ...
    else if(PlaystationGamePad.DOWN_KEY_TAP && selectedOption != 1)
    {
    	selectedOption++;
    }
...
void MenuDraw(GSGLOBAL* gsGlobal, u64 colour)
{
	...
	switch(selectedOption)
	{
		// Start New Game
		case 0:
			DrawText(180, 300, "YJDFZ BUHF",2, gsGlobal, &colour,1);
			DrawText(180, 325, "PFDTHIBNM BUHE",2, gsGlobal, &colour,0);
		break;
		case 1: 

			DrawText(180, 300, "YJDFZ BUHF",2, gsGlobal, &colour,0);
			DrawText(180, 325, "PFDTHIBNM BUHE",2, gsGlobal, &colour,1);
		break;
		default:
	}
